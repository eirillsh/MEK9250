...
def run_problem(N, mu_val, SUPG=True):
    ...
    right_bc = fem.dirichletbc(ScalarType(1.0), right_bc_dofs, V)
    ...
    def u_ex_f(x):
        return np.expm1(-x[0] / mu_val) / np.expm1(-1 / mu_val)
    u_ex = fem.Function(V)
    u_ex.interpolate(u_ex_f)
...
def u_func(x):
    return np.expm1(-x / mu) / np.expm1(-1 / mu)
